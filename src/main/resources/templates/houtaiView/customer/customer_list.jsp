...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
							...
							<tbody>
								<c:forEach items="${customers}" var="customer">
									<tr class="gradeX">
										...
										<td>${customer.customerLoginName}</td>
										<td>${customer.customerAge}</td>
										<td>${customer.customerSex}</td>
										<td>${customer.customerPhone}</td>
										<td>${customer.idCardNo}</td>
										<td><botton class="btn btn-info btn-xs"
												onclick="selectloginpassword(${customer.customerId})"
												data-toggle="modal" data-target="#select"> <span
												class="glyphicon glyphicon-tasks"></span> 查看</botton>
											<button class="btn btn-primary btn-xs"
												onclick="update(${customer.customerId})" data-toggle="modal"
												data-target="#myModal">
						...
						<!-- 查看 -->
						<div class="modal inmodal" id="select" tabindex="-1" role="dialog"
							aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content animated bounceInRight">
									...
									<div class="modal-header">
										...
										<h6 class="modal-title">用户账号密码</h6>
										<small class="font-bold"> <!-- 这里可以显示副标题。 -->
									</div>

									<form id="selectform" name="selectfrom">
										<div class="modal-body">
											...
											<div class="form-group">
												<label>姓名</label> <input type="text" readonly="readonly"
													class="form-control" id="name" name="customerName">
											</div>

											<div class="form-group">
												<label>账号</label> <input type="text" class="form-control"
													id="loginname" readonly="readonly" name="customerLoginName">
											</div>


											<div class="form-group">
												<label>密码</label> <input type="text" readonly="readonly"
													class="form-control" id="password" name="customerPassword"
													required="required">
											</div>
										...
										<div class="modal-body">
											...
											<div class="form-group">
												<label>帐号</label> <input type="text" placeholder="帐号"
													class="form-control" id="customer_login_name" name="customerLoginName" required="" >
											</div>
											
											<div class="form-group">
												<label>密码</label> <input type="text" placeholder="密码"
													class="form-control" id="customer_password" name="customerPassword" required="">
											</div>
											
											<div class="form-group">
												<label>年龄</label> <input type="number" placeholder="请输入年龄"
													class="form-control" id="customer_age" name="customerAge">
											</div>
											...
											<div class="form-group">
												...
												<label>性别</label> <br> <label> <input
													type="radio" id="customer_sex_woman" value="女"
													name="customerSex" />女
												</label> <label> <input type="radio" value="男"
													name="customerSex" id="customer_sex_man" />男
												</label>
											</div>
											...
											<div class="form-group">
												<label>电话号码</label> <input type="number" required="required"
													...
													id="customer_phone" name="customerPhone">
											</div>

											<div class="form-group">
												<label>身份证号码</label> <input type="number" placeholder="身份证"
													class="form-control" id="id_card_no" name="idCardNo">
											</div>
										</div>
	...
	<script>
			...
			//查看账号密码
			function selectloginpassword(id){
				console.log(id)
				$.post(getContextPath()+"/houtai/customer/update.do",
						{id:id},
						function(msg){	
							 $("#id").val(msg.customerId);
							$("#name").val(msg.customerName);
							$("#loginname").val(msg.customerLoginName);
							$("#password").val(msg.customerPassword);
						}
						);
			...
			function deleteuser(id){
				$.post(getContextPath()+"/houtai/customer/delete.do",
						{id:id},
						function(msg){		
							if(msg == 1){
								console.log(msg);
								setTimeout(function() {
									window.location.href=getContextPath()+"/houtai/customer/list.do";
								}, 500);
							}
						}
						);
    		...
    		// jquery 表单提交
   			$.post(getContextPath()+"/houtai/customer/doupdate.do",formJSON,function(msg){ 
					console.log(msg);
					if(msg == 1){
						window.location.href = getContextPath()+"/houtai/customer/list.do"; 
					}
				...
				$.post(getContextPath()+"/houtai/customer/update.do",
						...
						function(msg){	
							/*  console.log(msg)
							 ...
							 $("#customer_id").val(msg.customerId);
							$("#customer_name").val(msg.customerName);
							$("#customer_login_name").val(msg.customerLoginName);
							$("#customer_password").val(msg.customerPassword);
							$("#customer_age").val(msg.customerAge);
						if(msg.customerSex == "女"){
							$("#customer_sex_woman").attr("checked","chencked")
						}else{
							$("#customer_sex_man").attr("checked","chencked")
						}
						$("#customer_phone").val(msg.customerPhone);
						$("#id_card_no").val(msg.idCardNo);
						}
						);
